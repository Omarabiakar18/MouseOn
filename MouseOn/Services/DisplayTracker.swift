...
final class DisplayTracker: ObservableObject {
    ...
    /// Only touched from the main-thread event-monitor callbacks.
    nonisolated(unsafe) private var lastMouseMoveProcessed: TimeInterval = 0
    /// True while a trailing update is pending for events the throttle dropped.
    /// Only touched from the main thread (event monitors + main-queue block).
    nonisolated(unsafe) private var trailingUpdateScheduled: Bool = false
    ...
    /// Accessed from nonisolated deinit
    nonisolated(unsafe) private var displayObserver: Any?

    // Combine-based timers - accessed from nonisolated deinit
    nonisolated(unsafe) private var displayRefreshCancellable: AnyCancellable?
    nonisolated(unsafe) private var universalControlCancellable: AnyCancellable?
    ...
    private func updateCurrentDisplay() {
        ...
        // Skip the redundant writes if already flagged — this path repeats on
        // every mouse move while the cursor is off-screen, and @Published
        // notifies subscribers even when the value is unchanged
        guard !isOnUniversalControl else { return }
    ...
    private func createDisplayInfoFromCG(
        ...
        // CGDisplayBounds is in Quartz space (origin top-left, Y down); NSRect
        // comparisons against NSEvent.mouseLocation use Cocoa space (origin
        // bottom-left, Y up). Flip relative to the main display's height.
...
// MARK: - Mouse Move Throttling

extension DisplayTracker {

    /// Called for every raw mouse-move event (up to 120 Hz on ProMotion).
    /// Throttles to ~20 Hz so a Task and display lookup aren't spawned per event.
    /// Dropped events schedule a trailing update: the last move of a fast gesture
    /// is usually the one that lands on the new display, and nothing else would
    /// pick it up until the user moves the mouse again.
    nonisolated fileprivate func handleMouseMoved() {
        let now = ProcessInfo.processInfo.systemUptime
        let sinceLast = now - lastMouseMoveProcessed
        guard sinceLast >= Constants.Timing.mouseMoveThrottleInterval else {
            scheduleTrailingUpdate(after: Constants.Timing.mouseMoveThrottleInterval - sinceLast)
            return
        }
        lastMouseMoveProcessed = now
        Task { @MainActor [weak self] in
            self?.updateCurrentDisplay()
        }
    }

    /// Schedules a single deferred update once the throttle window closes, so the
    /// final event of a gesture isn't discarded. No-op if one is already pending.
    nonisolated private func scheduleTrailingUpdate(after delay: TimeInterval) {
        guard !trailingUpdateScheduled else { return }
        trailingUpdateScheduled = true
        DispatchQueue.main.asyncAfter(deadline: .now() + max(delay, 0)) { [weak self] in
            guard let self else { return }
            self.trailingUpdateScheduled = false
            self.lastMouseMoveProcessed = ProcessInfo.processInfo.systemUptime
            Task { @MainActor [weak self] in
                self?.updateCurrentDisplay()
            }
        }
