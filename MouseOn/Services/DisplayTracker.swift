...
final class DisplayTracker: ObservableObject {
    ...
    private func checkForUniversalControl() {
        ...
        let (shouldSwitchToFast, shouldSwitchToSlow, likelyOnUniversalControl, wasOnUniversalControl) = stateQueue.sync {
            ...
            // Use internal synchronized state, not @Published property
            let switchToFast = isAtScreenEdge && !isInFastPollingMode
            let switchToSlow = !isAtScreenEdge && isInFastPollingMode && !isOnUniversalControlInternal
            ...
            // Capture current internal state before potential update
            let wasOnUC = isOnUniversalControlInternal
            
            return (switchToFast, switchToSlow, likelyOnUC, wasOnUC)
        }
        
        // Handle polling mode switches outside of the sync block to avoid deadlock
        if shouldSwitchToFast {
            startUniversalControlTimer(fast: true)
        } else if shouldSwitchToSlow {
            startUniversalControlTimer(fast: false)
        }
        
        if likelyOnUniversalControl {
            if !wasOnUniversalControl {
