...
final class CursorHighlighter {
    ...
    func highlight(color: NSColor = .systemOrange) {
        ...
        let window = makeOverlayWindow(at: mouseLocation, size: windowSize, color: color)
    ...
    /// Create the borderless click-through overlay window centered on the cursor
    private func makeOverlayWindow(at mouseLocation: NSPoint, size windowSize: CGFloat, color: NSColor) -> NSWindow {
